...
**왜 DB functional index 를 골랐나**: long_url 은 TEXT(최대 2KB) 라 B-tree index 를 직접 걸면 크고 느리다. `md5(long_url)` 에 functional index 를 걸면 후보를 16-byte hash 매칭으로 좁힌 뒤 `long_url = ?` 로 md5 충돌 safety-net 까지 거는 2단 필터가 된다. 외부 라이브러리 의존성 없이 단일 DB 로 닫힌다는 점도 장점. 책 8장은 long_url 중복 검사 방법을 구체적으로 규정하지 않으며, 이 선택은 본 챕터에서 직접 내린 것. 후속 최적화로 Bloom filter 를 miss 경로 앞단에 두는 방안은 회고 이후 별도 실험으로 남겨두었다 (본 챕터 범위 밖, [Bloom filter 정리 노션 글](https://www.notion.so/33fccedc770981f1b031ebe9748ca108) 참조).
...
3. **Bloom filter 가 필요한 지점이 수치로 보인다** — miss 케이스의 −22% 중 대부분은 "DB SELECT 왕복" 에서 온다. Bloom filter 는 miss 의 대부분(실제로 DB 에 없는 경우)을 **DB 왕복 없이 기각** 할 수 있는 확률적 자료구조다. 결과적으로 dedup miss 의 오버헤드가 거의 0 에 수렴 → hit rate 가 낮은 워크로드에서도 dedup 이 손해가 아님. 이 관찰은 이 챕터의 실험 결과에서 **내가 떠올린 후속 최적화** 이지 책 8장이 권한 것이 아니다 (책 8장 원문에 Bloom filter 언급은 확인되지 않음). 개념 정리와 구현 스케치는 별도 노션 글 [📝 Bloom filter — 비트 배열로 만든 확률적 집합](https://www.notion.so/33fccedc770981f1b031ebe9748ca108) 에 작성했고, 이 챕터에서는 **[다음에 시도할 것]** 으로 남김.
...
- **Dedup 의 가치는 전적으로 hit rate 에 종속된다.** 0% hit 이면 -23%, 100% hit 이면 +164%. 워크로드를 모른 채 켜면 도박. 여기서 떠올린 후속 최적화가 Bloom filter — miss 케이스에서 "확실히 없음" 판정을 싸게 내려 DB 왕복을 건너뛰면, 낮은 hit rate 에서도 손해가 0 에 수렴하고 높은 hit rate 에서는 이득이 그대로 남는 **비대칭** 이 만들어진다. 책 8장이 이 방향을 제시한 것은 아니며, 실측 결과에서 이 챕터가 스스로 도출한 후속 실험 후보다.
...
- **Bloom filter 로 dedup miss 경로 최적화.** 이 챕터의 빈칸. `dedup-unique` 의 −23% 가 거의 0 으로 떨어지는지 확인. Guava `BloomFilter` 쓰거나 직접 구현 (비트 배열 + double hashing). 원소당 10 bits·FP 1% 로 잡으면 1천만 URL ≈ 12 MB 메모리. FP rate 0.01 → 0.001 로 낮추면 메모리 ~1.5× 로 얼마나 개선되는지. 개념 정리와 구현 스케치는 노션 [📝 Bloom filter](https://www.notion.so/33fccedc770981f1b031ebe9748ca108) 에 분리 작성.
